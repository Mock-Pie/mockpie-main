...
class TokenHandler:
    ...
    def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
        ...
        try:
            ...
            try:
                ...
                if not redis_client.validate_access_token(user.id, token):
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
                        detail=ErrorMessage.INVALID_TOKEN.value,
                        headers={"WWW-Authenticate": "Bearer"},
    ...
    @staticmethod
    def store_tokens_in_redis(user_id: Union[int, Any], access_token: str, refresh_token: str,
                             access_expires: timedelta, refresh_expires: timedelta):
        """
    ...
    @staticmethod
    def revoke_tokens(user_id: Union[int, Any]):
        """
