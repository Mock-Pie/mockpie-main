...
class EyeContactAnalyzer:
    ...
    async def analyze_eye_contact(self, video_path: str, sample_rate: float = 1.0) -> Dict[str, Any]:
        """Analyze eye contact patterns from video"""
        try:
            ...
            # Process results and return structured analysis
            return await self._process_eye_contact_results(results, fps, total_frames)
    ...
    async def _analyze_frame(self, frame: np.ndarray, timestamp: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if abs(deviation) < threshold:
                ...
                zone = "left"   # From speaker's perspective, viewer sees speaker looking left
            else:
                zone = "right"
    ...
    async def _process_eye_contact_results(self, results: List[Dict[str, Any]], fps: float, total_frames: int) -> Dict[str, Any]:
        """Process raw eye contact results into structured analysis"""
        try:
            if not results:
                return {
            ...
            # Pattern quality: more center attention = better pattern
            ...
            return {
                ...
                "raw_data": results[:10]  # Include first 10 frames for debugging
            }

        except Exception as e:
            logger.error(f"Error processing eye contact results: {e}")
